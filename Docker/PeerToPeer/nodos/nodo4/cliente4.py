...
def ver_estado_numeros_nodo4():
    try:
        response = requests.get("http://172.18.0.14:5004/ofrecer_numeros_nodo4")
        if response.status_code == 200:
            datos = response.json()
            print(f"Numero del nodo4: {datos['numero_nodo4']}")
            print(f"Sumatoria actual del nodo4: {datos['sumatoria_actual_del_nodo4']}")
        else:
            print("Error al obtener el estado de los numeros del nodo4")
    except Exception as e:
        print(f"Error: {e}")

def ver_estado_numeros_vecinos():
    nodos_vecinos = ["nodo1", "nodo6"]
    for nodo in nodos_vecinos:
        try:
            response = requests.get(f"http://172.18.0.14:5004/obtener_nodo{nodo[-1]}/sumatoria_actual")
            if response.status_code == 200:
                datos = response.json()
...
def obtener_y_sumar_numero(nodo):
    try:
        response = requests.get(f"http://172.18.0.14:5004/obtener_nodo{nodo[-1]}/sumar_su_numero")
        if response.status_code == 200:
            datos = response.json()
            print(datos["mensaje"])
            print(f"Sumatoria actual del nodo4: {datos['sumatoria_actual_del_nodo4']}")
