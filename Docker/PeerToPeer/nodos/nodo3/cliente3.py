...
def mostrar_menu():
    ...
    print("3. Obtener y sumar numero del nodo2")
    print("4. Obtener y sumar sumatoria actual nodo2")
    print("5. Obtener y sumar numero del nodo5")
    print("6. Obtener y sumar sumatoria actual nodo5")
    print("7. Salir")

def ver_estado_numeros_nodo3():
    try:
        response = requests.get("http://172.18.0.13:5003/ofrecer_numeros_nodo3")
        if response.status_code == 200:
            datos = response.json()
            print(f"Numero del nodo3: {datos['numero_nodo3']}")
            print(f"Sumatoria actual del nodo3: {datos['sumatoria_actual_del_nodo3']}")
        else:
            print("Error al obtener el estado de los numeros del nodo3")
    except Exception as e:
        print(f"Error: {e}")

def ver_estado_numeros_vecinos():
    nodos_vecinos = ["nodo2", "nodo5"]
    for nodo in nodos_vecinos:
        try:
            response = requests.get(f"http://172.18.0.13:5003/obtener_nodo{nodo[-1]}/sumatoria_actual")
            if response.status_code == 200:
                datos = response.json()
...
def obtener_y_sumar_numero(nodo):
    try:
        response = requests.get(f"http://172.18.0.13:5003/obtener_nodo{nodo[-1]}/sumar_su_numero")
        if response.status_code == 200:
            datos = response.json()
            print(datos["mensaje"])
            print(f"Sumatoria actual del nodo3: {datos['sumatoria_actual_del_nodo3']}")
